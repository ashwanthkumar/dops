...
func init() {
	server.Register("dops-http", constructor)
}
...
func (s *httpServer) routes() *httprouter.Router {
	...
	r.GET("/blobs/:digest/torrent", s.serveTorrentBlobs)
	return r
}

func (s *httpServer) serveTorrentBlobs(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	fmt.Fprintf(w, "Serving MagnetUrl for %s!\n", p.ByName("digest"))
}
